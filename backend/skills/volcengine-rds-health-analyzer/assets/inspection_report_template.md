...
> - 无论是在聊天中直接展示，还是保存为 Markdown 文件，都必须从本模板标题开始输出，不要在前后追加导语、总结、额外小节或自由发挥的补充内容。
> - 如果某个占位符对应节点拆分内容，必须填充固定列表头 `Node | Avg | Median | Max | 风险级别 | 说明` 的 Markdown 表格。
> - 不要把模板内容包在 Markdown 代码块、引用块或其他额外容器里，也不要把模板表格改写成列表、段落或 JSON。
...
> - 若本次分析同时包含最近 `24h` 辅助窗口判断，也只能写入模板现有占位符位置，并明确标注这是“最近 24 小时辅助窗口”或“recent24h”结论。
> - 当主窗口正常、最近 `3d` 正常、但最近 `24h` 异常时，应表述为“最新出现的短时异常”或同等语义；不要让最近 `24h` 的短窗口结论覆盖主窗口的总体健康判断。
